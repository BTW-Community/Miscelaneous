...
public class JBGuiNameNetherMap extends GuiScreen {
	/**
	...
	public JBGuiNameNetherMap(EntityPlayer ep, int scale, float x, float z) {
	...
	public void onGuiClosed() {
		...
		if (this.currentName.trim().length()==0)
			this.currentName = "Soul Map ("+scale+"x)";
		else 
		...
		try {
			...
			var5.writeFloat(x);
			var5.writeFloat(z);
			Packet250CustomPayload packet = new Packet250CustomPayload("JB|CNM", var4.toByteArray());
